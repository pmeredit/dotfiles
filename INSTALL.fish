...
for i in (ls | grep -v INSTALL.sh | grep -v apt-cyg | grep -v INSTALL.fish | grep -v omf.fish | grep -v omf.fish | grep -v README.md | grep -v pipeformatter.py | grep -v bin | grep -v random)
    echo "stowing $i ..."
    stow $i
end
